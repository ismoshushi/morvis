...
class ReplyClient(private val prefs: Prefs) {
    ...
    fun draft(snapshot: ChatSnapshot, relationship: String, ctx: ChatContext? = null): List<String> {
        ...
        // Tone preset: appended to the system prompt so all three candidates are
        // written in the chosen voice while still differing in strategy.
        val styleHint = prefs.styleLine()
