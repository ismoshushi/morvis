...
open class ChatCaptureService : AccessibilityService() {
    ...
    private val adapters = listOf(
        WeChatAdapter(), QQAdapter(), XRouterAdapter(), FeishuAdapter(), SoulAdapter()
    ).associateBy { it.pkg }
    ...
    /**
     * Settings saved → reconcile the live state at once. Before this, a switch
     * flipped in Settings only took effect whenever the service happened to be
     * restarted, so the bubble kept offering "分析当前对话" after auto-analysis
     * had been turned off.
     */
    private val settingsReceiver = object : BroadcastReceiver() {
        override fun onReceive(c: Context?, i: Intent?) {
            Log.i(TAG, "settings changed → reloading")
            main.post {
                if (!prefs.enabled) {
                    overlay?.hide()
                    return@post
                }
                overlay?.refreshFromPrefs()
                // Switched back on (or resized): make sure the bubble is there.
                if (overlay?.isShowing() != true) runCatching { maybeCapture() }
            }
        }
    }
    ...
    override fun onServiceConnected() {
        ...
        runCatching {
            if (Build.VERSION.SDK_INT >= 33) {
                registerReceiver(settingsReceiver, IntentFilter(Prefs.ACTION_SETTINGS_CHANGED),
    ...
    private fun maybeCapture() {
        ...
        val adapter = adapters[pkg] ?: run {
            Log.d(TAG, "extract[$pkg] 没有适配器，跳过"); return
        }
        // Only act inside a chat window (the adapter returns null elsewhere).
        val rawSnapshot = adapter.extract(root, resources)
        Log.d(TAG, "extract[$pkg] " +
            (rawSnapshot?.let { "命中 n=${it.messages.size} title=${it.title}" } ?: "null（判定不在会话页）"))
        val snapshot0 = rawSnapshot ?: return
        ...
        val snapshot = stabilizeTitle(pkg ?: "", snapshot0)
        ...
        // Feeds ([ChatAppAdapter.manualOnly], e.g. the X timeline) never trigger:
        // scrolling would re-analyze every few hundred milliseconds.
        if (adapter.manualOnly || snapshot.latestFrom != "other" || !prefs.autoAnalyze) {
    ...
    private fun runAnalysis() {
        ...
        if (analyzing) return
        if (!prefs.hasKey()) {
            val what = if (prefs.judgeEnabled) "判断接口密钥" else "回复接口密钥"
            main.post { overlay?.showError("未设置$what，去设置里填") }
            return
        }
        analyzing = true
        ...
        submit {
            ...
            // Judgment is fast (~1s) — show it immediately. Skipped entirely
            // when the judge route is off: the panel would just be empty, and
            // the call can only fail (no /decisions on a relay).
            if (prefs.judgeEnabled) {
                submit {
    ...
    private fun fillInput(text: String) {
        submit {
            ...
            if (!ok) {
                ...
                // Never clicks send.
                val edit = rootInActiveWindow?.let { resolveInput(it) }
                if (edit != null) {
                    ...
                    if (!ok) {
                        ...
                        val focused = rootInActiveWindow?.let { resolveInput(it) } ?: edit
                        setTextRaw(focused, "")
    ...
    private fun trySetText(text: String): Boolean {
        val edit = rootInActiveWindow?.let { resolveInput(it) } ?: return false
    ...
    private fun readInput(): String? {
        val edit = rootInActiveWindow?.let { resolveInput(it) } ?: return null
        runCatching { edit.refresh() }
        return edit.text?.toString()
    }

    /**
     * The node to type a reply into. Apps that draw their own input box (Soul's
     * `id/et_sendmessage` is a plain View, not an EditText) name it through
     * [ChatAppAdapter.inputNode]; everyone else falls back to the first
     * editable node in the tree.
     */
    private fun resolveInput(root: AccessibilityNodeInfo): AccessibilityNodeInfo? {
        val adapter = root.packageName?.toString()?.let { adapters[it] }
        adapter?.inputNode(root)?.let { return it }
    ...
    companion object {
        ...
        private val TRANSIENT_TITLE_WORDS = listOf(
            ...
            // Soul swaps the nickname out for its typing indicator; X-style
            // placeholders live here too, so a capture taken mid-typing cannot
            // file "正在输入…" as a contact name.
            "正在输入", "Typing"
        )
    }
}
