...
interface ChatAppAdapter {
    ...
    /**
     * Feed surfaces (the X timeline) rather than conversations: content scrolls
     * past continuously, so auto-analysis would fire on every scroll and burn
     * tokens on posts the user is not looking at. True means: show the idle
     * bubble and wait for a tap, never analyze on our own.
     */
    val manualOnly: Boolean get() = false

    /**
     * The node a chosen reply is typed into, for apps whose input box is not a
     * real EditText: Soul draws its own (`id/et_sendmessage` is a plain
     * `android.view.View`), so the service's "first editable node in the tree"
     * search finds nothing there and the fill would always fall back to the
     * clipboard. null (the default) keeps that generic search.
     */
    fun inputNode(root: AccessibilityNodeInfo): AccessibilityNodeInfo? = null
}
...
/**
 * Soul (cn.soulapp.android) private messages. Measured on device 2026-09-22
 * (uiautomator dump, 1156x2510, cn.soulapp.android.component.chat.ConversationActivity).
 *
 * The tree is NOT obfuscated and every node carries a resource-id, so this is
 * the easy case: no disguised service (WeChat) and no contentDescription
 * guessing (X) needed.
 *
 * One conversation row is `id/item_root`:
 *   id/chat_avatar                  sender's avatar, pinned to that sender's
 *                                   outer edge — others at x≈15 of 1156, me at
 *                                   x≈964 (Soul mirrors the layout for me)
 *   id/container                    the bubble
 *     id/content_text_container
 *       id/content_text             the message body (TextView)
 *   id/message_read                 a small dot, only ever on MY bubbles
 *
 * "In a chat window" = the input box `id/et_sendmessage` AND the conversation
 * RecyclerView `id/list_conversation` are both in the tree. Soul reuses ids
 * across screens, so neither signal alone is enough.
 *
 * Side comes from the avatar column, never from the bubble's centre: my bubbles
 * stop at x≈952 of 1156, but a long incoming bubble can still push its centre
 * past mid-screen. `id/message_read` is the fallback (mine only), and the
 * bubble's own centre is the last resort.
 */
class SoulAdapter : ChatAppAdapter {
    ...
    override fun extract(root: AccessibilityNodeInfo, res: Resources): ChatSnapshot? {
        ...
        while (stack.isNotEmpty() && guard < 6000) {
            ...
            when (node.viewIdResourceName) {
                TITLE_ID -> node.text?.toString()?.let {
                    if (title == null && it.isNotBlank()) title = it.trim()
                }
                INPUT_ID -> hasInput = true
                LIST_ID -> hasList = true
                ITEM_ROOT_ID -> {
                    readRow(node, width)?.let { rows.add(it) }
                    descend = false // the row's own children are already read
                }
            }
            if (descend) {
                for (i in node.childCount - 1 downTo 0) node.getChild(i)?.let { stack.addLast(it) }
        ...
        // Not a conversation. Soul puts the input box only in a thread, and the
        // conversation list only inside one; anything else is another screen.
        if (!hasInput || !hasList) return null

        // Soul drops `tv_title` entirely while the other person is typing and
        // puts a "正在输入…" strip in its place, so fall back to Soul's own title
        // search (which knows that strip is chrome, not a name).
        if (title == null) {
            title = findSoulTitle(root, rows.firstOrNull()?.top ?: Int.MAX_VALUE, width, res)
        }
        // In a thread with nothing readable — image/voice only, or a thread the
        // tree has not populated yet — is the OCR fallback's cue.
        if (rows.isEmpty()) return ChatSnapshot(title, emptyList())
    ...
    /**
     * Soul's own title search, used when `id/tv_title` is missing.
     *
     * The generic [findTitleInActionBar] cannot be used here: Soul's action bar
     * holds a cluster of chrome — the online badge, the follow button, and the
     * "正在输入…" strip that REPLACES the nickname while the other side is
     * typing — all of it short, topmost and left-aligned, i.e. exactly the
     * shape that search calls a title. Measured: it happily returned
     * "正在输入…" from a real dump.
     */
    private fun findSoulTitle(
        root: AccessibilityNodeInfo,
        firstRowTop: Int,
        width: Int,
        res: Resources
    ): String? {
        val limit = minOf(firstRowTop, (res.displayMetrics.heightPixels * 0.14).toInt())
        val maxCenterX = (width * 0.7).toInt()
        val stack = ArrayDeque<AccessibilityNodeInfo>()
        stack.addLast(root)
        var best: String? = null
        var bestTop = Int.MAX_VALUE
        ...
        while (stack.isNotEmpty() && guard < 5000) {
            ...
            val id = node.viewIdResourceName
            val text = node.text?.toString()?.trim()
            if (!text.isNullOrEmpty() && id !in CHROME_IDS &&
                text.length <= 24 && !looksLikeTimestamp(text)
            ) {
                val b = Rect(); node.getBoundsInScreen(b)
                if (b.bottom in 1 until limit && b.centerX() in 0..maxCenterX && b.top < bestTop) {
                    bestTop = b.top; best = text
                }
            }
            for (i in node.childCount - 1 downTo 0) node.getChild(i)?.let { stack.addLast(it) }
        }
    ...
    /**
     * One `id/item_root` subtree → one message. Null for rows that carry no
     * text (the date divider row, image and voice bubbles).
     */
    private fun readRow(item: AccessibilityNodeInfo, width: Int): SoulRow? {
        val box = Rect()
        item.getBoundsInScreen(box)
        ...
        while (stack.isNotEmpty() && guard < 400) {
            ...
            when (node.viewIdResourceName) {
                CONTENT_TEXT_ID -> {
                    ...
                    if (body == null && !t.isNullOrEmpty()) {
                        body = t
                        val b = Rect(); node.getBoundsInScreen(b); bodyCenterX = b.centerX()
                    }
                }
                AVATAR_ID -> {
                    if (avatarCenterX < 0) {
                        val b = Rect(); node.getBoundsInScreen(b); avatarCenterX = b.centerX()
                    }
                }
                READ_ID -> mine = true
            }
        ...
        val side = when {
            avatarCenterX >= 0 -> if (avatarCenterX > width / 2) "me" else "other"
            mine -> "me"
            bodyCenterX >= 0 -> if (bodyCenterX > width / 2) "me" else "other"
            else -> "other"
        }
        return SoulRow(box.top, side, text)
    }

    /** Soul's input box is a drawn View, not an EditText — name it explicitly
     *  or the service's editable-node search cannot find anything to type into. */
    override fun inputNode(root: AccessibilityNodeInfo): AccessibilityNodeInfo? {
        val stack = ArrayDeque<AccessibilityNodeInfo>()
        stack.addLast(root)
        var guard = 0
        while (stack.isNotEmpty() && guard < 6000) {
            guard++
            val node = stack.removeLast()
            if (node.viewIdResourceName == INPUT_ID) return node
            for (i in node.childCount - 1 downTo 0) node.getChild(i)?.let { stack.addLast(it) }
        }
        return null
    }

    private data class SoulRow(val top: Int, val side: String, val text: String)

    companion object {
        /** Action-bar nodes that are not the conversation title — see [findSoulTitle]. */
        private val CHROME_IDS = setOf(
            "cn.soulapp.android:id/tv_online_label",
            "cn.soulapp.android:id/tv_input_status",
            "cn.soulapp.android:id/static_input_status",
            "cn.soulapp.android:id/chat_state",
            "cn.soulapp.android:id/chat_follow_btn",
            "cn.soulapp.android:id/item_left_back",
            "cn.soulapp.android:id/chat_convsation_more"
        )
        private const val ITEM_ROOT_ID = "cn.soulapp.android:id/item_root"
        private const val CONTENT_TEXT_ID = "cn.soulapp.android:id/content_text"
        private const val AVATAR_ID = "cn.soulapp.android:id/chat_avatar"
        private const val READ_ID = "cn.soulapp.android:id/message_read"
        private const val TITLE_ID = "cn.soulapp.android:id/tv_title"
        private const val INPUT_ID = "cn.soulapp.android:id/et_sendmessage"
        private const val LIST_ID = "cn.soulapp.android:id/list_conversation"
    }
}
