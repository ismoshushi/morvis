...
class MainActivity : AppCompatActivity() {
    ...
    private fun build() {
        ...
        container.addView(text("在聊天 App 旁读对方消息（已支持微信、QQ、X、飞书、Soul），给出判断和候选回复。发送始终由你手动点。",
        ...
        toggle.setOnClickListener {
            ...
            // Let the running service know: the bubble has to appear/disappear
            // now, not whenever the next accessibility event happens to arrive.
            sendBroadcast(Intent(Prefs.ACTION_SETTINGS_CHANGED).setPackage(packageName))
            build()
        }
