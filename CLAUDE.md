...
挂在任意聊天 App 旁边（微信、QQ、X、飞书、Soul 已适配）的非侵入式助手：读到对方最新消息 → 调 Jev 判断 → 悬浮窗给出分析和 3 条候选回复（Jev 排序）→ 人一键填入微信输入框。**发送永远由人手动点，程序不自动发。**
...
- Soul（`cn.soulapp.android`，2026-09-22 实测，1156×2510 / ConversationActivity 停在前台）：**节点不混淆、全部带 resource-id**，普通无障碍服务即可读，**不需要伪装服务**（和 X 一样是最省心的一类）。私信会话行 `id/item_root` 的结构：正文 `id/content_text`（TextView）、头像 `id/chat_avatar`、`id/message_read`（小红点，**只出现在我的气泡上**）。**分边看头像列，不看气泡中心**——对方头像 left≈15、我的 left≈964（屏宽 1156），但长消息的气泡中心会越过半屏。标题 `id/tv_title` 是**左对齐**（centerX≈183 ≈ 0.16 屏宽）。**对方"正在输入"时 Soul 会把 `tv_title` 整个撤掉**，换成 `id/chat_state` + `id/tv_input_status`（文案"正在输入…"）——实测第二屏 dump 里 `tv_title` 出现 0 次。所以 Soul **不能用通用 `findTitleInActionBar` 兜底**：那一屏它会把"正在输入…"当成会话标题（顶栏还有"在线""关注""关注后可邀请通话"同样落在命中范围）。适配器自带 `findSoulTitle()`，按 id 排除这批顶栏 chrome；`ChatCaptureService.TRANSIENT_TITLE_WORDS` 也加了"正在输入"，双保险。
- Soul 的**输入框 `id/et_sendmessage` 是 `android.view.View`，不是 EditText**（实测全树 0 个 `android.widget.EditText`，键盘弹起后仍是 View）。所以服务的 `findEditable()` 在 Soul 里永远返回 null，"填入"会退化成"已复制，长按粘贴"。为此 `ChatAppAdapter` 增加了可选方法 `inputNode(root)`，适配器直接点名输入框，服务用 `resolveInput()` 优先取它、取不到才回退找可编辑节点。
- Soul 自带 AI 助手（`小助手将提供实时聊天建议`）会**往输入框预填建议文案**（实测出现过 `上 soul 都去干啥`、`幽默感能瞬间破冰`）。SET_TEXT 是整体覆盖，不会拼接；但这也意味着输入框默认非空，任何"输入框为空才算没填"的假设都不成立。发送按钮 `id/btn_send`，**绝不点**。
...
- **配置写入必须用 `Prefs.put { }`（内部 `commit()` 同步落盘），不许用 `apply()`**（2026-09-22 用户实测：填好密钥、划掉 App 再进就空了，SP 里只剩 `bubble_x/y`）。`apply()` 是异步排队写，进程被划掉/杀掉时队列可能还没落盘。配置量小、只在用户操作时写，同步写这点开销是对的。
- **离开设置页即保存**：`SettingsActivity.onPause()` 调 `saveAction(false)`，"保存全部设置"不再是唯一落盘路径；保存后发 `Prefs.ACTION_SETTINGS_CHANGED`（带 `setPackage`），`ChatCaptureService` 动态注册接收并 `overlay.refreshFromPrefs()`，主开关 `MainActivity` 也发同一个广播。没有它，设置里关掉的开关要等服务重启才生效——这就是"关闭分析对话，悬浮球还提示分析对话"的根因。
- **悬浮窗大小可拖**：面板底部右下角 `⤡` 把手，拖动改宽/高，松手存 `prefs.overlayWidth/overlayHeight`（-1 = 默认 316dp 宽 / 40% 屏高）。窗口是 `WRAP_CONTENT`，改完必须 `wm.updateViewLayout` 才会重新测量。设置页"外观"里有"恢复默认大小"。
- 这台机器上 **adb 没有 `INJECT_EVENTS` 权限**（`input tap` / `keyevent` 抛 `SecurityException`），且 `SettingsActivity` 未导出（`am start -n` 被 `Permission Denial: not exported`）。真机验证配置持久化走 `src/debug` 的 `PrefsPersistCheckActivity`（**只在 debug 构建里，release 不含**）：`am start -n <pkg>/com.jev.probe.debug.PrefsPersistCheckActivity --es v sk-xxx` → `am force-stop` → `run-as <pkg> cat shared_prefs/jev_assistant.xml`。
